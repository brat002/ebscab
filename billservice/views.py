...
def get_promise(request):
    ...
    last_promise = Transaction.objects.filter(account=user, type=TransactionType.objects.get(internal_name='PROMISE_PAYMENT')).order_by('-created')
    if last_promise:
...
def change_tariff(request):
    ...
    if request.method == 'POST':
        ...
        suppagreements = AccountSuppAgreement.objects.filter(Q(closed__isnull=True) | 
                               Q(closed__gte=now), account=request.user.account, created__lte=now, suppagreement__disable_tarff_change=True)
        if suppagreements:
            return {
